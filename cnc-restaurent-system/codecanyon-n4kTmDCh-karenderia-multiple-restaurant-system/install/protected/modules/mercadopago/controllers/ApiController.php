...
class ApiController extends CController
{
    ...
    public function actioncreatecheckout()
    {
        try {
            ...
            $order_meta = COrders::getAttributesAll($order->order_id,['custom_scheme']);
            $custom_scheme = $order_meta['custom_scheme'] ?? APP_CUSTOM_URL_SCHEME;                        

            if($request_from=="app"){
                if(!empty($return_url)){
                    $redirect_url = $return_url;
                } else $redirect_url = $custom_scheme."://payment-callback?status=cancel";
            } else {
    ...
    public function actionverifypayment()
    {
        try {
            ...
            $order_meta = COrders::getAttributesAll($data->order_id,['custom_scheme']);
            $custom_scheme = $order_meta['custom_scheme'] ?? APP_CUSTOM_URL_SCHEME;        
            ...
            if( $status=="null" || empty($status)){                
                ...
                if($request_from=="app"){
                    if(!empty($return_url)){
                        $back_url = $return_url;
                    } else $back_url = $custom_scheme."://payment-callback?status=home";
                } else {
            ...
            if($request_from=="app"){
                ...
                } else $redirect_url = $custom_scheme."://payment-callback?".http_build_query(['status'=>'successful','order_id'=>$order_uuid]);
    ...
    public function actionprocesspayment()
    {
        try {
            ...
            $custom_scheme = $decoded['custom_scheme'] ?? APP_CUSTOM_URL_SCHEME;
        ...
        try {
            ...
            if($model){
                ...
                if($request_from=="app"){
                    if(!empty($return_url)){
                        $redirect_url = $return_url;
                    } else $redirect_url = $custom_scheme."://payment-callback?".http_build_query([
