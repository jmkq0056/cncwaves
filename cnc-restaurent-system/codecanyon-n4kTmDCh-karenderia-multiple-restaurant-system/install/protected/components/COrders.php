...
class COrders
{
	...
	private static $is_commission_applied;
	
	public static function setCommissionapplied($is_commission_applied=false)
	{
		self::$is_commission_applied = $is_commission_applied;
	}

	public static function getCommissionapplied()
	{
		return self::$is_commission_applied;
	}
	...
	public static function getOrder($order_id='', $lang = KMRS_DEFAULT_LANGUAGE)
	{
		...
		$is_commission_applied = self::getCommissionapplied();
		...
		(
		 select GROUP_CONCAT(item_row,';',subcat_id,';',sub_item_id,';',qty,';',multi_option,';',price,';',addons_total,';',merchant_price)
			...
			foreach ($res as $val) {				
				...
				if(is_array($_addon_items) && count($_addon_items)>=1){
					foreach ($_addon_items as $val3) {						
						...
						if($is_commission_applied){							
							$price = is_array($addonitems) ? ($addonitems[7] ?? 0) : 0;
				...
				$data[]=array(
				   ...
				   'price'=> $is_commission_applied ? floatval($val['merchant_price']) :  floatval($val['price']),
	...
	{		
		...
		if( $res = Yii::app()->db->createCommand($stmt)->queryRow() ){
			...
			$res['restaurant_name'] = CommonUtility::safeDecode($res['restaurant_name']);
    ...
    public static function paymentHistory($order_id=0)
    {
    	...
    	if($model){
    		...
    		foreach ($model as $item) {    		
				...
				if($is_commission_applied){
					$trans_amount = ($item->merchant_amount*self::$exchange_rate);
				} else $trans_amount = ($item->trans_amount*self::$exchange_rate);				
    			$data[] = array( 
    			   'is_online'=>$is_online,    			   
    			   ...
    			   'transaction_type'=>$item->transaction_type,
    			   'transaction_description'=>t($item->transaction_description),
    			   'trans_amount'=>Price_Formatter::formatNumber( $trans_amount),
