   ...
   <a href="#section-address" class="d-block chevron center position-relative no-hover">
	...
	<div class="text-center">
		...
		<?php if($home_search_mode=="address"):?>
			...
			<component-merchant-services ref="ref_services" @after-update="afterUpdateServices" :label="{ min:'<?php echo CJavaScript::quote(t("min"))?>' }"></component-merchant-services>
			</div>
		<?php else : ?>
			<div style="display:none;">
			<components-location-estimation ref="ref_location_services"></components-location-estimation>
			</div>
		<?php endif;?>
    </div>
  ...
  <div class="row">
	...
	<div class="description">
	  ...
	  if(!empty($data['description'])){
		echo Yii::app()->input->xssClean(nl2br($data['description'])); 
	  }
	  ?>	    
    </div>
