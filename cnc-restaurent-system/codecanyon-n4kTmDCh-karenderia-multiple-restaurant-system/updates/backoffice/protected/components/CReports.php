...
class CReports {
	...
	public static function dailySalesSummary($merchant_id=0, $start='',$end='',$status=array(),$payment_list=array())
	{
		$in_payment = '';
		$in_status = CommonUtility::arrayToQueryParameters($status);
		if($payment_list){			
			$paymentFilter = CommonUtility::arrayToQueryParameters($payment_list);
			$in_payment = "AND a.payment_code IN (".$paymentFilter.")";
		}		
		$stmt = "				
