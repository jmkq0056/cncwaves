...
{	
	...
	public static function setOriginaltotal($value=0){
		self::$total_original = floatval($value);
	}

	public static function getOriginaltotal(){
		return floatval(self::$total_original);
	}
	...
	{
		// Bug fix (2026-06-12, order #10051): the language filter previously
		// dropped EVERY row for non-default-language sessions, leaving the cart
		// with no price data → items persisted at price=0 → 4 DKK bag fee was
		// the only charge → customer got ~6 free meals.
		//
		// Robust fix: don't filter by language in the SQL. Pull all translation
		// rows for the cart's item_size_ids, then in PHP prefer the row matching
		// the requested language; otherwise fall back to whatever is available
		// (default-lang row, then any). Price/discount columns are identical
		// across translation rows (they come from st_item_relationship_size,
		// not st_size_translation), so it's safe.
		$stmt="
		SELECT a.item_size_id,a.size_name,a.language,
		a.price , a.discount,
		a.discount_type,
		 (
		...
		if($res = Yii::app()->db->createCommand($stmt)->queryAll()){
			$preferred = array();
			$fallback = array();
			$default_lang = defined('KMRS_DEFAULT_LANGUAGE') ? KMRS_DEFAULT_LANGUAGE : '';
			foreach ($res as $val) {
				$key = $val['item_size_id'];
				$row_lang = isset($val['language']) ? $val['language'] : '';
				if ($row_lang === $lang || $row_lang === '') {
					$preferred[$key] = $val;
				} elseif (!isset($fallback[$key]) || $row_lang === $default_lang) {
					$fallback[$key] = $val;
				}
			}
			$data = array();
			foreach ($preferred + $fallback as $key => $val) {
				$row = isset($preferred[$key]) ? $preferred[$key] : $val;
				$data[$key] = array(
				  'item_size_id'=>$row['item_size_id'],
				  'size_name'=>$row['size_name'],
				  'price'=>$row['price'],
				  'discount'=>$row['discount_valid']>0?$row['discount']:0,
				  'discount_type'=>$row['discount_type'],
				  'discount_valid'=>$row['discount_valid'],
				);
			}
			if ($data) return $data;
		}
    ...
    {    	
    	...
    	if(!CCart::isEmpty()){
    		...
    		if ( $condition = CCart::getCondition()){
    			...
    			foreach ($condition as $val) {    				    				    				
    				if($val['target']=="subtotal"){          
						$max_discount_cap = $val['max_discount_cap'] ?? 0;						
    					$value = CCart::summary($val['value'],$sub_total_without_cnd , $max_discount_cap);    					
    					$results[] = array(
    					 'name'=>$val['name'],
    ...
    public static function apply($total=0, $condition_val=0,$max_discount_cap=0)
    {    	
    	$results = 0;    	
    	if ( CCart::valueIsPercentage($condition_val)){
			...
			if($max_discount_cap>0){
				if($raw_value>$max_discount_cap){
					$raw_value = $max_discount_cap;
				}
			}
    ...
    public static function summary($condition_val=0,$total=0,$max_discount_cap=0)
    {    	
    	$results = '';  
    	$raw_value = (float) CCart::cleanValue($condition_val);     	
    	if ( CCart::valueIsPercentage($condition_val)){    		    		
	...
	public static function getCartTransactionType($cart_uuid='',$merchant_id='')
	{
		$transaction_type = '';
		if($cart_uuid){
			$transaction_type = CCart::cartTransaction($cart_uuid,Yii::app()->params->local_transtype,$merchant_id);			
		} else {
			$transaction_resp = CCheckout::getFirstTransactionType($merchant_id,Yii::app()->language);			
			$transaction_type = $transaction_resp ? $transaction_resp : 'delivery';
		}				
		return $transaction_type;
	}

	public static function validateItemPromo($item_token='',$cart_uuid='')
	{
		$stmt = "
		SELECT a.item_token
		FROM {{item_free_promo}} a	
		WHERE a.item_token IN (
		...
		if($res = $command->queryRow()){			
			throw new Exception( t("Item Promo already added") );
		}
		return true;
	}
