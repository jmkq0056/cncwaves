...
{	
	...
	public function actionupdate_item_available()
	{
		...
		if($model){
			// Bug fix (2026-06-30): scenario assignment was BEFORE the null
			// guard, causing fatal "method on null" 500s when a stale row id
			// hit this endpoint. And `next_action: "silent"` swallowed the
			// success message so the merchant saw "nothing happen" even on
			// successful toggle. Now: scenario inside the guard + a real
			// success message + an explicit `available` value the JS can
			// reflect on the row.
			...
			$this->code = 1;
			$this->msg = $checked
				? t("Item is now available")
				: t("Item marked as sold out");
			$this->details = array(
				'item_id'   => $item_id,
				'available' => (integer)$checked,
			);
