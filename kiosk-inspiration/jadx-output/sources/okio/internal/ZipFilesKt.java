...
public final class ZipFilesKt {
    ...
    public static final ZipFileSystem openZip(Path zipPath, FileSystem fileSystem, Function1<? super ZipEntry, Boolean> predicate) throws IOException {
        ...
        try {
            fileHandle = bufferedSourceOpenReadOnly;
            size = fileHandle.size() - ((long) 22);
        } finally {
        }
        if (size < 0) {
        ...
        do {
            ...
            try {
                if (bufferedSourceBuffer.readIntLe() == END_OF_CENTRAL_DIRECTORY_SIGNATURE) {
                    ...
                    if (j2 > 0) {
                        bufferedSourceOpenReadOnly = Okio.buffer(fileHandle.source(j2));
                        try {
    ...
    private static final Map<Path, ZipEntry> buildIndex(List<ZipEntry> list) {
        Path path = Path.Companion.get$default(Path.INSTANCE, RemoteSettings.FORWARD_SLASH_STRING, false, 1, (Object) null);
