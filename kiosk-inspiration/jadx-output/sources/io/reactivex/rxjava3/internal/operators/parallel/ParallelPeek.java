...
public final class ParallelPeek<T> extends ParallelFlowable<T> {
    ...
    public ParallelPeek(ParallelFlowable<T> source, Consumer<? super T> onNext, Consumer<? super T> onAfterNext, Consumer<? super Throwable> onError, Action onComplete, Action onAfterTerminated, Consumer<? super Subscription> onSubscribe, LongConsumer onRequest, Action onCancel) {
        this.source = source;
        this.onNext = (Consumer) Objects.requireNonNull(onNext, "onNext is null");
        this.onAfterNext = (Consumer) Objects.requireNonNull(onAfterNext, "onAfterNext is null");
        this.onError = (Consumer) Objects.requireNonNull(onError, "onError is null");
        this.onComplete = (Action) Objects.requireNonNull(onComplete, "onComplete is null");
        this.onAfterTerminated = (Action) Objects.requireNonNull(onAfterTerminated, "onAfterTerminated is null");
        this.onSubscribe = (Consumer) Objects.requireNonNull(onSubscribe, "onSubscribe is null");
        this.onRequest = (LongConsumer) Objects.requireNonNull(onRequest, "onRequest is null");
        this.onCancel = (Action) Objects.requireNonNull(onCancel, "onCancel is null");
    }
    ...
    public void subscribe(Subscriber<? super T>[] subscribers) {
        if (validate(subscribers)) {
            ...
            for (int i = 0; i < length; i++) {
                subscriberArr[i] = new ParallelPeekSubscriber(subscribers[i], this);
            }
