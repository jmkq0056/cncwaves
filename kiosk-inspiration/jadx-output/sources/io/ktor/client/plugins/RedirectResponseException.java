...
public final class RedirectResponseException extends ResponseException {
    ...
    public RedirectResponseException(HttpResponse response, String cachedResponseText) {
        ...
        this.message = "Unhandled redirect: " + response.getCall().getRequest().getMethod().getValue() + TokenParser.SP + response.getCall().getRequest().getUrl() + ". Status: " + response.getStatus() + ". Text: \"" + cachedResponseText + '\"';
    }

    /* JADX WARN: 'this' call moved to the top of the method (can break code semantics) */
    @Deprecated(level = DeprecationLevel.ERROR, message = "Please, provide response text in constructor")
    public RedirectResponseException(HttpResponse response) {
