...
public final class CoroutineScheduler implements Executor, Closeable {
    ...
    */
    ...
    public final void dispatch(Runnable block, TaskContext taskContext, boolean tailDispatch) {
        ...
        long jAddAndGet = z2 ? controlState$volatile$FU.addAndGet(this, PARKED_VERSION_INC) : 0L;
    ...
    private final Task submitToLocalQueue(Worker worker, Task task, boolean z) {
        if (worker == null || worker.state == WorkerState.TERMINATED || (task.taskContext.getTaskMode() == 0 && worker.state == WorkerState.BLOCKING)) {
            return task;
        }
            ...
            if (worker != null) {
                ...
                if (i7 == 1) {
                    ...
                    if (size$kotlinx_coroutines_core > 0) {
                        arrayList.add(new StringBuilder().append(size$kotlinx_coroutines_core).append(GMTDateParser.DAY_OF_MONTH).toString());
                    }
                } else if (i7 == 5) {
                    i5++;
                }
