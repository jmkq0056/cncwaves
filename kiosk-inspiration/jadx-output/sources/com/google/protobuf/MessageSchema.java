...
final class MessageSchema<T> implements Schema<T> {
    ...
    private static long offset(int value) {
        return value & WinNT.NLS_VALID_LOCALE_MASK;
    }
    ...
    */
    ...
    public int getSerializedSize(T message) {
        ...
        T t = message;
        Unsafe unsafe = UNSAFE;
        int i2 = WinNT.NLS_VALID_LOCALE_MASK;
        int i3 = 0;
        int i4 = 0;
        int i5 = 0;
        int i6 = 1048575;
        while (i3 < messageSchema.buffer.length) {
            ...
            i3 += 3;
            i2 = WinNT.NLS_VALID_LOCALE_MASK;
        }
    ...
    */
    ...
    private int parseOneofField(T message, byte[] data, int position, int limit, int tag, int number, int wireType, int typeAndOffset, int fieldType, long fieldOffset, int bufferPosition, ArrayDecoders.Registers registers) throws IOException {
        Unsafe unsafe = UNSAFE;
        long j = this.buffer[bufferPosition + 2] & WinNT.NLS_VALID_LOCALE_MASK;
        switch (fieldType) {
            case 51:
                if (wireType != 1) {
                    return position;
    ...
    */
    ...
    private boolean isOneofPresent(T message, int fieldNumber, int pos) {
        return UnsafeUtil.getInt(message, (long) (presenceMaskAndOffsetAt(pos) & WinNT.NLS_VALID_LOCALE_MASK)) == fieldNumber;
    }

    private boolean isOneofCaseEqual(T message, T other, int pos) {
        long jPresenceMaskAndOffsetAt = presenceMaskAndOffsetAt(pos) & WinNT.NLS_VALID_LOCALE_MASK;
    ...
    private void setOneofPresent(T message, int fieldNumber, int pos) {
        UnsafeUtil.putInt(message, presenceMaskAndOffsetAt(pos) & WinNT.NLS_VALID_LOCALE_MASK, fieldNumber);
    }
