...
final class RateLimiter {
    ...
    boolean isEventRateLimited(PerfMetric perfMetric) {
        boolean zCheck;
        if (!isRateLimitApplicable(perfMetric)) {
            return false;
        }
        if (perfMetric.hasNetworkRequestMetric()) {
            zCheck = this.networkLimiter.check(perfMetric);
        } else {
            if (!perfMetric.hasTraceMetric()) {
                return true;
            }
            zCheck = this.traceLimiter.check(perfMetric);
        }
        return !zCheck;
    }

    boolean isEventSampled(PerfMetric perfMetric) {
