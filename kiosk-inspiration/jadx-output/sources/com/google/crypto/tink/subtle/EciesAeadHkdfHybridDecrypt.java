...
public final class EciesAeadHkdfHybridDecrypt implements HybridDecrypt {
    ...
    public byte[] decrypt(final byte[] ciphertext, final byte[] contextInfo) throws GeneralSecurityException {
        ...
        return this.demHelper.getAead(this.recipientKem.generateKey(Arrays.copyOfRange(ciphertext, 0, iEncodingSizeInBytes), this.hkdfHmacAlgo, this.hkdfSalt, contextInfo, this.demHelper.getSymmetricKeySizeInBytes(), this.ecPointFormat)).decrypt(Arrays.copyOfRange(ciphertext, iEncodingSizeInBytes, ciphertext.length), EMPTY_AAD);
    }
}
