...
abstract class AbstractCatchingFuture<V, X extends Throwable, F, T> extends FluentFuture.TrustedFuture<V> implements Runnable {
    ...
    protected String pendingToString() {
        ...
        if (listenableFuture == null) {
            ...
            string = new StringBuilder(String.valueOf(strValueOf).length() + 16).append("inputFuture=[").append(strValueOf).append("], ").toString();
        }
        if (cls != null && f != null) {
            String strValueOf2 = String.valueOf(cls);
            String strValueOf3 = String.valueOf(f);
            return new StringBuilder(String.valueOf(string).length() + 29 + String.valueOf(strValueOf2).length() + String.valueOf(strValueOf3).length()).append(string).append("exceptionType=[").append(strValueOf2).append("], fallback=[").append(strValueOf3).append("]").toString();
        }
        if (strPendingToString == null) {
            return null;
        }
        String strValueOf4 = String.valueOf(string);
        String strValueOf5 = String.valueOf(strPendingToString);
