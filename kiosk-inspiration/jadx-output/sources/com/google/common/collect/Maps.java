...
public final class Maps {
    ...
    private static <K, V> void doDifference(Map<? extends K, ? extends V> map, Map<? extends K, ? extends V> map2, Equivalence<? super V> equivalence, Map<K, V> map3, Map<K, V> map4, Map<K, V> map5, Map<K, MapDifference.ValueDifference<V>> map6) {
        for (Map.Entry<? extends K, ? extends V> entry : map.entrySet()) {
            ...
            if (map2.containsKey(key)) {
                R.anim animVar = (Object) NullnessCasts.uncheckedCastNullableTToT(map4.remove(key));
                if (equivalence.equivalent(value, animVar)) {
                    map5.put(key, value);
                } else {
                    map6.put(key, ValueDifferenceImpl.create(value, animVar));
                }
            } else {
                map3.put(key, value);
            }
    ...
    static String toStringImpl(Map<?, ?> map) {
        StringBuilder sbAppend = Collections2.newStringBuilderForCollection(map.size()).append(AbstractJsonLexerKt.BEGIN_OBJ);
        boolean z = true;
    ...
    static abstract class EntrySet<K, V> extends Sets.ImprovedAbstractSet<Map.Entry<K, V>> {
        ...
        public boolean contains(@CheckForNull Object obj) {
            if (obj instanceof Map.Entry) {
                ...
                if (Objects.equal(objSafeGet, entry.getValue()) && (objSafeGet != null || map().containsKey(key))) {
                    return true;
                }
            }
            return false;
