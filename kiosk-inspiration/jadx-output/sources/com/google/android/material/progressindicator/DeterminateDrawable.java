        ...
        if (!getBounds().isEmpty() && isVisible() && canvas.getClipBounds(rect)) {
            ...
            if (this.baseSpec.indicatorTrackGapSize > 0) {
                if (this.drawingDelegate instanceof LinearDrawingDelegate) {
                    iClamp = this.baseSpec.indicatorTrackGapSize;
                } else {
                    iClamp = (int) ((this.baseSpec.indicatorTrackGapSize * MathUtils.clamp(getIndicatorFraction(), 0.0f, GAP_RAMP_DOWN_THRESHOLD)) / GAP_RAMP_DOWN_THRESHOLD);
                }
