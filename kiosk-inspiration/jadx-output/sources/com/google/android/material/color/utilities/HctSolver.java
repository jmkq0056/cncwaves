...
public class HctSolver {
    ...
    static double[] nthVertex(double d, int i) {
        ...
        if (i < 8) {
            ...
            if (isBounded(d8)) {
                return new double[]{d6, d8, d5};
            }
            return new double[]{-1.0d, -1.0d, -1.0d};
        }
        double d9 = ((d - (d2 * d5)) - (d3 * d6)) / d4;
        if (isBounded(d9)) {
            return new double[]{d5, d6, d9};
        }
        return new double[]{-1.0d, -1.0d, -1.0d};
    }
    ...
    static int findResultByJ(double d, double d2, double d3) {
        ...
        while (i < 5) {
            ...
            double d8 = d5;
            double d9 = dSqrt;
            double dPow2 = Math.pow(((d2 == 0.0d || dSqrt == 0.0d) ? 0.0d : d2 / Math.sqrt(d7)) * dPow, 1.1111111111111112d);
            double aw = (viewingConditions.getAw() * Math.pow(d7, (d6 / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double d10 = (((0.305d + aw) * 23.0d) * dPow2) / (((23.0d * dCos) + ((dPow2 * 11.0d) * dCos2)) + ((108.0d * dPow2) * dSin));
            double d11 = d10 * dCos2;
            double d12 = d10 * dSin;
            double d13 = aw * 460.0d;
            double[] dArrMatrixMultiply = MathUtils.matrixMultiply(new double[]{inverseChromaticAdaptation(((d13 + (451.0d * d11)) + (288.0d * d12)) / 1403.0d), inverseChromaticAdaptation(((d13 - (891.0d * d11)) - (261.0d * d12)) / 1403.0d), inverseChromaticAdaptation(((d13 - (d11 * 220.0d)) - (d12 * 6300.0d)) / 1403.0d)}, LINRGB_FROM_SCALED_DISCOUNT);
            double d14 = dArrMatrixMultiply[0];
            if (d14 < 0.0d) {
                break;
            }
            double d15 = dArrMatrixMultiply[1];
            if (d15 < 0.0d) {
                break;
            }
            double d16 = dArrMatrixMultiply[2];
            if (d16 < 0.0d) {
                break;
            }
            double[] dArr = Y_FROM_LINRGB;
            double d17 = (dArr[0] * d14) + (dArr[1] * d15) + (dArr[2] * d16);
            if (d17 <= 0.0d) {
                return 0;
            }
            if (i != 4) {
