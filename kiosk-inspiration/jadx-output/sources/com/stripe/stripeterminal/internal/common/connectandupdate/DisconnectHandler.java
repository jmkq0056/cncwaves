...
public final class DisconnectHandler extends ConnectAndUpdateStateHandler {
    ...
    @Inject
    public DisconnectHandler(ReaderConnectionController readerConnectionController) {
        super(ConnectAndUpdateState.DISCONNECT);
    ...
    public void onEnter(ConnectAndUpdateApplicationData current, ConnectAndUpdateState from) {
        ...
        Reader device;
        if ((from != ConnectAndUpdateState.EMPTY && from != ConnectAndUpdateState.CONNECT) || current == null || (connectionSummary = current.getConnectionSummary()) == null || (device = connectionSummary.getDevice()) == null) {
            return;
        }
        this.readerConnectionController.disconnect(device);
    }
}
