...
public final class MainlandModule_Companion_ProvideRestClientFactory implements Factory<RestClient> {
    ...
    public RestClient get() {
        return provideRestClient(this.okHttpClientProvider.get(), this.serviceUrlProvider.get(), this.idempotencyRetryInterceptorProvider.get(), this.sessionTokenInterceptorProvider.get(), this.customRestInterceptorProvider.get(), this.traceLoggingRestClientInterceptorProvider.get(), this.logWriterProvider.get());
    }

    public static MainlandModule_Companion_ProvideRestClientFactory create(Provider<OkHttpClientProvider> provider, Provider<RestClient.BaseUrlProvider> provider2, Provider<IdempotencyRetryInterceptor> provider3, Provider<SessionTokenInterceptor> provider4, Provider<RestInterceptor> provider5, Provider<TraceLoggingRestClientInterceptor> provider6, Provider<LogWriter> provider7) {
    ...
    public static RestClient provideRestClient(OkHttpClientProvider okHttpClientProvider, RestClient.BaseUrlProvider baseUrlProvider, IdempotencyRetryInterceptor idempotencyRetryInterceptor, SessionTokenInterceptor sessionTokenInterceptor, RestInterceptor restInterceptor, TraceLoggingRestClientInterceptor traceLoggingRestClientInterceptor, LogWriter logWriter) {
        return (RestClient) Preconditions.checkNotNullFromProvides(MainlandModule.INSTANCE.provideRestClient(okHttpClientProvider, baseUrlProvider, idempotencyRetryInterceptor, sessionTokenInterceptor, restInterceptor, traceLoggingRestClientInterceptor, logWriter));
    }
}
