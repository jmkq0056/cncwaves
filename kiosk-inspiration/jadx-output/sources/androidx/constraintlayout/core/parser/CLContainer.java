...
public class CLContainer extends CLElement {
    ...
    public CLContainer mo35clone() {
        CLContainer cLContainer = (CLContainer) super.mo35clone();
        ArrayList<CLElement> arrayList = new ArrayList<>(this.mElements.size());
        Iterator<CLElement> it = this.mElements.iterator();
        while (it.hasNext()) {
            CLElement cLElementMo35clone = it.next().mo35clone();
            cLElementMo35clone.setContainer(cLContainer);
            arrayList.add(cLElementMo35clone);
        }
