...
final class DecodedBitStreamParser {
    ...
    private static int getCountry(byte[] bArr) {
        return getInt(bArr, new byte[]{53, 54, 43, 44, BufferUtil.MINUS, 46, 47, 48, 37, 38});
    }
    ...
    private static int getInt(byte[] bArr, byte[] bArr2) {
        ...
        for (int i4 = 0; i4 < bArr2.length; i4++) {
            bit += getBit(bArr2[i4], bArr) << ((bArr2.length - i4) - 1);
        }
        return bit;
    }
    ...
    private static String getMessage(byte[] bArr, int i4, int i5) {
        ...
        int i9 = 0;
        while (i6 < i4 + i5) {
            char cCharAt = SETS[i7].charAt(bArr[i6]);
            switch (cCharAt) {
                case 65520:
                case 65521:
                case 65522:
    ...
    private static int getPostCode2(byte[] bArr) {
        return getInt(bArr, new byte[]{33, 34, 35, 36, 25, 26, 27, 28, 29, 30, 19, 20, 21, 22, 23, 24, 13, 14, 15, 16, 17, 18, 7, 8, 9, 10, Flags.CD, 12, 1, 2});
    }
    ...
    private static String getPostCode3(byte[] bArr) {
        String[] strArr = SETS;
        return String.valueOf(new char[]{strArr[0].charAt(getInt(bArr, new byte[]{39, 40, 41, 42, 31, 32})), strArr[0].charAt(getInt(bArr, new byte[]{33, 34, 35, 36, 25, 26})), strArr[0].charAt(getInt(bArr, new byte[]{27, 28, 29, 30, 19, 20})), strArr[0].charAt(getInt(bArr, new byte[]{21, 22, 23, 24, 13, 14})), strArr[0].charAt(getInt(bArr, new byte[]{15, 16, 17, 18, 7, 8})), strArr[0].charAt(getInt(bArr, new byte[]{9, 10, Flags.CD, 12, 1, 2}))});
    }

    private static int getServiceClass(byte[] bArr) {
        return getInt(bArr, new byte[]{55, 56, 57, HttpTokens.COLON, HttpTokens.SEMI_COLON, 60, 49, 50, 51, 52});
    }
}
