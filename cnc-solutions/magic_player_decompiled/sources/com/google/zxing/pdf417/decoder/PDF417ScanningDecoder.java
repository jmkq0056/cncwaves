...
public final class PDF417ScanningDecoder {
    ...
    */
    ...
    private static DecoderResult createDecoderResult(DetectionResult detectionResult) throws NotFoundException {
        ...
        for (int i4 = 0; i4 < detectionResult.getBarcodeRowCount(); i4++) {
            ...
            while (i5 < detectionResult.getBarcodeColumnCount()) {
                int i6 = i5 + 1;
                int[] value = barcodeValueArrCreateBarcodeMatrix[i4][i6].getValue();
    ...
    private static DecoderResult createDecoderResultFromAmbiguousValues(int i4, int[] iArr, int[] iArr2, int[] iArr3, int[][] iArr4) throws ChecksumException {
    ...
    public static DecoderResult decode(BitMatrix bitMatrix, ResultPoint resultPoint, ResultPoint resultPoint2, ResultPoint resultPoint3, ResultPoint resultPoint4, int i4, int i5) throws NotFoundException {
        ...
        while (true) {
            if (resultPoint != null) {
                rowIndicatorColumn = getRowIndicatorColumn(bitMatrix, boundingBox, resultPoint, true, i4, i5);
            }
            if (resultPoint3 != null) {
                rowIndicatorColumn2 = getRowIndicatorColumn(bitMatrix, boundingBox, resultPoint3, false, i4, i5);
            }
        ...
        boolean z4 = rowIndicatorColumn != null;
        int iMin = i4;
        int iMax = i5;
        for (int i9 = 1; i9 <= barcodeColumnCount; i9++) {
            int i10 = z4 ? i9 : barcodeColumnCount - i9;
            if (detectionResultMerge.getDetectionResultColumn(i10) == null) {
                ...
                while (minY <= boundingBox.getMaxY()) {
                    int startColumn = getStartColumn(detectionResultMerge, i10, minY, z4);
                    ...
                    i6 = i12;
                    int i13 = minY;
                    Codeword codewordDetectCodeword = detectCodeword(bitMatrix, boundingBox.getMinX(), boundingBox.getMaxX(), z4, i8, i13, iMin, iMax);
                    i7 = i13;
                    if (codewordDetectCodeword != null) {
                        detectionResultRowIndicatorColumn.setCodeword(i7, codewordDetectCodeword);
    ...
    private static DecoderResult decodeCodewords(int[] iArr, int i4, int[] iArr2) throws ChecksumException, FormatException {
        if (iArr.length == 0) {
            throw FormatException.getFormatInstance();
        }
        int i5 = 1 << (i4 + 1);
        int iCorrectErrors = correctErrors(iArr, iArr2, i5);
        verifyCodewordCount(iArr, i5);
        DecoderResult decoderResultDecode = DecodedBitStreamParser.decode(iArr, String.valueOf(i4));
    ...
    private static Codeword detectCodeword(BitMatrix bitMatrix, int i4, int i5, boolean z3, int i6, int i7, int i8, int i9) {
        ...
        int decodedValue;
        int codeword;
        int iAdjustCodewordStartColumn = adjustCodewordStartColumn(bitMatrix, i4, i5, z3, i6, i7);
        int[] moduleBitCount = getModuleBitCount(bitMatrix, i4, i5, z3, iAdjustCodewordStartColumn, i7);
        if (moduleBitCount == null) {
            return null;
        }
        int iSum = MathUtils.sum(moduleBitCount);
        if (z3) {
            i10 = iAdjustCodewordStartColumn + iSum;
        } else {
            ...
            i10 = iAdjustCodewordStartColumn;
        }
        if (checkCodewordSkew(iSum, i8, i9) && (codeword = PDF417Common.getCodeword((decodedValue = PDF417CodewordDecoder.getDecodedValue(moduleBitCount)))) != -1) {
            return new Codeword(iAdjustCodewordStartColumn, i10, getCodewordBucketNumber(decodedValue), codeword);
        }
        return null;
    }
    ...
    private static DetectionResultRowIndicatorColumn getRowIndicatorColumn(BitMatrix bitMatrix, BoundingBox boundingBox, ResultPoint resultPoint, boolean z3, int i4, int i5) {
        ...
        while (i6 < 2) {
            ...
            for (int y3 = (int) resultPoint.getY(); y3 <= boundingBox.getMaxY() && y3 >= boundingBox.getMinY(); y3 += i7) {
                Codeword codewordDetectCodeword = detectCodeword(bitMatrix, 0, bitMatrix.getWidth(), z3, x3, y3, i4, i5);
                if (codewordDetectCodeword != null) {
                    detectionResultRowIndicatorColumn.setCodeword(y3, codewordDetectCodeword);
                    x3 = z3 ? codewordDetectCodeword.getStartX() : codewordDetectCodeword.getEndX();
                }
            }
            i6++;
        }
    ...
    private static int getStartColumn(DetectionResult detectionResult, int i4, int i5, boolean z3) {
        ...
        while (true) {
            ...
            for (Codeword codeword2 : detectionResult.getDetectionResultColumn(i4).getCodewords()) {
                if (codeword2 != null) {
                    return ((codeword2.getEndX() - codeword2.getStartX()) * i6 * i8) + (z3 ? codeword2.getEndX() : codeword2.getStartX());
                }
            }
            i8++;
        }
