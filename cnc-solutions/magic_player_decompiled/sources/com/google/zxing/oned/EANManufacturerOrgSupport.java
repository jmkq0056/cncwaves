...
final class EANManufacturerOrgSupport {
    ...
    private synchronized void initIfNeeded() {
        if (this.ranges.isEmpty()) {
            ...
            add(new int[]{60, WKSRecord.Service.NETBIOS_SSN}, "US/CA");
            add(new int[]{HttpStatus.MULTIPLE_CHOICES_300, 379}, "FR");
            add(new int[]{380}, "BG");
            add(new int[]{383}, "SI");
            add(new int[]{385}, "HR");
            add(new int[]{387}, "BA");
            add(new int[]{HttpStatus.BAD_REQUEST_400, 440}, "DE");
            ...
            add(new int[]{HttpStatus.INTERNAL_SERVER_ERROR_500, 509}, "GB");
            add(new int[]{520}, "GR");
    ...
    public String lookupCountryIdentifier(String str) {
        ...
        int i4;
        initIfNeeded();
        int i5 = Integer.parseInt(str.substring(0, 3));
        int size = this.ranges.size();
        for (int i6 = 0; i6 < size && i5 >= (i4 = (iArr = this.ranges.get(i6))[0]); i6++) {
            if (iArr.length != 1) {
                i4 = iArr[1];
            }
            if (i5 <= i4) {
                return this.countryIdentifiers.get(i6);
            }
        }
        return null;
    }
