...
public final class MultiFormatUPCEANReader extends OneDReader {
    ...
    public Result decodeRow(int i4, BitArray bitArray, Map<DecodeHintType, ?> map) throws NotFoundException {
        ...
        for (UPCEANReader uPCEANReader : this.readers) {
            try {
                Result resultDecodeRow = uPCEANReader.decodeRow(i4, bitArray, iArrFindStartGuardPattern, map);
                boolean z3 = resultDecodeRow.getBarcodeFormat() == BarcodeFormat.EAN_13 && resultDecodeRow.getText().charAt(0) == '0';
                Collection collection = map == null ? null : (Collection) map.get(DecodeHintType.POSSIBLE_FORMATS);
                boolean z4 = collection == null || collection.contains(BarcodeFormat.UPC_A);
                if (!z3 || !z4) {
