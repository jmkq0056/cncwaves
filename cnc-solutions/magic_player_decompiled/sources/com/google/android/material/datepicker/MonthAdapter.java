...
class MonthAdapter extends BaseAdapter {
    ...
    private boolean isSelected(long j4) {
        ...
        while (it.hasNext()) {
            if (UtcDates.canonicalYearMonthDay(j4) == UtcDates.canonicalYearMonthDay(it.next().longValue())) {
                return true;
            }
        }
        return false;
    ...
    private void updateSelectedState(@Nullable TextView textView, long j4) {
        CalendarItemStyle calendarItemStyle;
        if (textView == null) {
            return;
        }
        if (this.calendarConstraints.getDateValidator().isValid(j4)) {
            textView.setEnabled(true);
            calendarItemStyle = isSelected(j4) ? this.calendarStyle.selectedDay : UtcDates.getTodayCalendar().getTimeInMillis() == j4 ? this.calendarStyle.todayDay : this.calendarStyle.day;
        } else {
    ...
    private void updateSelectedStateForDate(MaterialCalendarGridView materialCalendarGridView, long j4) {
        if (Month.create(j4).equals(this.month)) {
            updateSelectedState((TextView) materialCalendarGridView.getChildAt(materialCalendarGridView.getAdapter().dayToPosition(this.month.getDayOfMonth(j4)) - materialCalendarGridView.getFirstVisiblePosition()), j4);
        }
    }
    ...
    public long getItemId(int i4) {
        return i4 / this.month.daysInWeek;
    }
    ...
    public boolean isFirstInRow(int i4) {
        return i4 % this.month.daysInWeek == 0;
    }

    public boolean isLastInRow(int i4) {
        return (i4 + 1) % this.month.daysInWeek == 0;
    }
    ...
    public int positionToDay(int i4) {
        return (i4 - this.month.daysFromStartOfWeekToFirstOfMonth()) + 1;
    }
    ...
    public boolean withinMonth(int i4) {
        return i4 >= firstPositionInMonth() && i4 <= lastPositionInMonth();
    }

    @Override // android.widget.Adapter
    @Nullable
    public Long getItem(int i4) {
        if (i4 < this.month.daysFromStartOfWeekToFirstOfMonth() || i4 > lastPositionInMonth()) {
            return null;
        }
