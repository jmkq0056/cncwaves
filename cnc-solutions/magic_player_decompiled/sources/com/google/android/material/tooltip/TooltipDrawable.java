...
public class TooltipDrawable extends MaterialShapeDrawable implements TextDrawableHelper.TextDrawableDelegate {
    ...
    private EdgeTreatment createMarkerEdge() {
        float f4 = -calculatePointerOffset();
        double dWidth = getBounds().width();
        double d4 = this.arrowSize;
        double dSqrt = Math.sqrt(2.0d);
        Double.isNaN(d4);
        Double.isNaN(dWidth);
        float f5 = ((float) (dWidth - (dSqrt * d4))) / 2.0f;
        return new OffsetEdgeTreatment(new MarkerEdgeTreatment(this.arrowSize), Math.min(Math.max(f4, -f5), f5));
    }
    ...
    private void loadFromAttributes(@Nullable AttributeSet attributeSet, @AttrRes int i4, @StyleRes int i5) {
        TypedArray typedArrayObtainStyledAttributes = ThemeEnforcement.obtainStyledAttributes(this.context, attributeSet, R.styleable.Tooltip, i4, i5, new int[0]);
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        canvas.translate(fCalculatePointerOffset, (float) (-(d5 - d6)));
        super.draw(canvas);
        drawText(canvas);
        canvas.restore();
    }
    ...
    public void setRevealFraction(@FloatRange(from = 0.0d, to = 1.0d) float f4) {
        ...
        this.tooltipScaleX = f4;
        this.tooltipScaleY = f4;
        this.labelOpacity = AnimationUtils.lerp(0.0f, 1.0f, 0.19f, 1.0f, f4);
        invalidateSelf();
    }
