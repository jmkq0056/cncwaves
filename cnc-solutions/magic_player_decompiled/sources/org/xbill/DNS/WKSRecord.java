...
public class WKSRecord extends Record {
    ...
    public void rdataFromString(Tokenizer tokenizer, Name name) throws TextParseException {
        ...
        if (iValue < 0) {
            throw a.a("Invalid IP protocol: ", string, tokenizer);
        }
        ...
        while (true) {
            ...
            if (iValue2 < 0) {
                StringBuffer stringBufferA = e.a("Invalid TCP/UDP service: ");
                stringBufferA.append(token.value);
                throw tokenizer.exception(stringBufferA.toString());
            }
            arrayList.add(new Integer(iValue2));
        }
    }
    ...
    public String rrToString() {
        ...
        for (int i4 = 0; i4 < this.services.length; i4++) {
            ...
            stringBufferA.append(this.services[i4]);
    ...
    public WKSRecord(Name name, int i4, long j4, InetAddress inetAddress, int i5, int[] iArr) {
        super(name, 11, i4, j4);
        if (Address.familyOf(inetAddress) != 1) {
            throw new IllegalArgumentException("invalid IPv4 address");
        }
        this.address = inetAddress.getAddress();
        this.protocol = Record.checkU8("protocol", i5);
        for (int i6 : iArr) {
            Record.checkU16(NotificationCompat.CATEGORY_SERVICE, i6);
        }
