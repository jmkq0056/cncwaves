...
public class DrawerArrowDrawable extends Drawable {
    ...
    public void draw(Canvas canvas) {
        ...
        float fLerp4 = lerp(z3 ? 0.0f : -180.0f, z3 ? 180.0f : 0.0f, this.mProgress);
        double d4 = fLerp;
        double d5 = fLerp3;
        double dCos = Math.cos(d5);
        Double.isNaN(d4);
        boolean z4 = z3;
        float fRound2 = Math.round(dCos * d4);
        double dSin = Math.sin(d5);
        Double.isNaN(d4);
        float fRound3 = Math.round(dSin * d4);
        this.mPath.rewind();
        float fLerp5 = lerp(this.mPaint.getStrokeWidth() + this.mBarGap, -this.mMaxCutForBarSize, this.mProgress);
        float f5 = (-fLerp2) / 2.0f;
        this.mPath.moveTo(f5 + fRound, 0.0f);
        this.mPath.rLineTo(fLerp2 - (fRound * 2.0f), 0.0f);
        this.mPath.moveTo(f5, fLerp5);
        ...
        if (this.mSpin) {
            canvas.rotate(fLerp4 * (this.mVerticalMirror ^ z4 ? -1 : 1));
        } else if (z4) {
    ...
    public void setBarThickness(float f4) {
        if (this.mPaint.getStrokeWidth() != f4) {
            ...
            Double.isNaN(d4);
            this.mMaxCutForBarSize = (float) (dCos * d4);
            invalidateSelf();
        }
    }
