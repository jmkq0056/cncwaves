...
class DocumentsContractApi19 {
    ...
    private static int queryForInt(Context context, Uri uri, String str, int i4) {
        return (int) queryForLong(context, uri, str, i4);
    }

    private static long queryForLong(Context context, Uri uri, String str, long j4) {
        Cursor cursorQuery = null;
        try {
            cursorQuery = context.getContentResolver().query(uri, new String[]{str}, null, null, null);
            return (!cursorQuery.moveToFirst() || cursorQuery.isNull(0)) ? j4 : cursorQuery.getLong(0);
        } catch (Exception e4) {
            Log.w("DocumentFile", "Failed query: " + e4);
            return j4;
        } finally {
