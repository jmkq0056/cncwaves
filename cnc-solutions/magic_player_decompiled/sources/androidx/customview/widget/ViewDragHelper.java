...
public class ViewDragHelper {
    ...
    private boolean checkNewEdgeDrag(float f4, float f5, int i4, int i5) {
        ...
        if (fAbs >= fAbs2 * 0.5f || !this.mCallback.onEdgeLock(i5)) {
            return (this.mEdgeDragsInProgress[i4] & i5) == 0 && fAbs > ((float) this.mTouchSlop);
        }
    ...
    private int computeAxisDuration(int i4, int i5, int i6) {
        ...
        float f4 = width / 2;
        float fDistanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(1.0f, Math.abs(i4) / width)) * f4) + f4;
        int iAbs = Math.abs(i5);
        return Math.min(iAbs > 0 ? Math.round(Math.abs(fDistanceInfluenceForSnapDuration / iAbs) * 1000.0f) * 4 : (int) (((Math.abs(i4) / i6) + 1.0f) * 256.0f), MAX_SETTLE_DURATION);
    }
    ...
    private int computeSettleDuration(View view, int i4, int i5, int i6, int i7) {
        ...
        int iClampMag2 = clampMag(i7, (int) this.mMinVelocity, (int) this.mMaxVelocity);
        int iAbs = Math.abs(i4);
        ...
        return (int) ((computeAxisDuration(i5, iClampMag2, this.mCallback.getViewVerticalDragRange(view)) * (f6 / f7)) + (computeAxisDuration(i4, iClampMag, this.mCallback.getViewHorizontalDragRange(view)) * f8));
    }
    ...
    private void reportNewEdgeDrags(float f4, float f5, int i4) {
        boolean zCheckNewEdgeDrag = checkNewEdgeDrag(f4, f5, i4, 1);
        ...
        if (r04 != 0) {
            ...
            iArr[i4] = iArr[i4] | r04;
            this.mCallback.onEdgeDragStarted(r04, i4);
        }
    }
    ...
    public boolean canScroll(@NonNull View view, boolean z3, int i4, int i5, int i6, int i7) {
        int i8;
    ...
    public void captureChildView(@NonNull View view, int i4) {
        if (view.getParent() != this.mParentView) {
            StringBuilder sbA = f.a("captureChildView: parameter must be a descendant of the ViewDragHelper's tracked parent view (");
            sbA.append(this.mParentView);
            sbA.append(")");
    ...
    public void processTouchEvent(@NonNull MotionEvent motionEvent) {
        ...
        if (actionMasked == 0) {
            ...
            View viewFindTopChildUnder = findTopChildUnder((int) x3, (int) y3);
        ...
        if (actionMasked == 2) {
            ...
            while (i5 < pointerCount) {
                ...
                if (isValidPointerForActionMove(pointerId2)) {
                    ...
                    if (this.mDragState != 1) {
                        View viewFindTopChildUnder2 = findTopChildUnder((int) x5, (int) y5);
                        if (checkTouchSlop(viewFindTopChildUnder2, f4, f5) && tryCaptureViewForDrag(viewFindTopChildUnder2, pointerId2)) {
                            break;
                        }
                    } else {
                        break;
        ...
        if (this.mDragState == 1 && pointerId4 == this.mActivePointerId) {
            ...
            while (true) {
                ...
                if (pointerId5 != this.mActivePointerId) {
                    View viewFindTopChildUnder3 = findTopChildUnder((int) motionEvent.getX(i5), (int) motionEvent.getY(i5));
    ...
    */
    ...
    public boolean smoothSlideViewTo(@NonNull View view, int i4, int i5) {
        this.mCapturedView = view;
        this.mActivePointerId = -1;
        boolean zForceSettleCapturedViewAt = forceSettleCapturedViewAt(i4, i5, 0, 0);
    ...
    private float clampMag(float f4, float f5, float f6) {
        ...
        return fAbs > f6 ? f4 > 0.0f ? f6 : -f6 : f4;
    }
