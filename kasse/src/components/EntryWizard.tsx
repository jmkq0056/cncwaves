...
export type EntryWizardProps = {
  /** "admin" (default) keeps the full dashboard behaviour; "staff" locks to a single day, no drafts, custom save. */
  mode?: "admin" | "staff";
  /** Staff mode: the only date that can be filled in. */
  fixedDate?: string;
  /** Persists the entry. Defaults to the admin POST /api/entries flow. */
  onSave?: (form: FormData) => Promise<SaveResult>;
  /** Staff mode: called when the day turns out to be already closed mid-flow. */
  onAlreadyDone?: () => void;
  /** Staff mode: called from the success screen ("log out"). */
  onExit?: () => void;
};

async function defaultAdminSave(form: FormData): Promise<SaveResult> {
  try {
...
export default function EntryWizard({ mode = "admin", fixedDate, onSave, onAlreadyDone, onExit }: EntryWizardProps) {
  const staff = mode === "staff";
  const router = useRouter();
  const searchParams = useSearchParams();
  const [idx, setIdx] = useState(0);
  const [form, setForm] = useState<FormData>(() =>
    staff ? { ...EMPTY, date: fixedDate || todayCPH(), employeeName: "" } : { ...EMPTY }
  );
  ...
  const [singleDayMode, setSingleDayMode] = useState(staff);
  ...
  // Auto-save draft on every change (admin only — staff must re-enter the PIN each time).
  useEffect(() => {
    if (staff || !draftLoaded.current || !ready) return;
    saveDraft(form, idx, rangeFrom, rangeTo, rangeDays, singleDayMode);
  }, [staff, form, idx, rangeFrom, rangeTo, rangeDays, singleDayMode, ready]);

  useEffect(() => {
    // Staff: always a fresh single day, no drafts, no history lookups.
    if (staff) {
      const d = fixedDate || todayCPH();
      setSingleDayMode(true);
      setRangeFrom(d); setRangeTo(d); setRangeDays(1);
  ...
  const next = useCallback(() => { setReturnIdx(null); setIdx((i) => Math.min(i + 1, STEPS.length - 1)); setMenuOpen(false); }, []);
  const back = useCallback(() => {
    setMenuOpen(false);
    if (returnIdx !== null) { setIdx(returnIdx); setReturnIdx(null); return; }
  ...
  async function handleSave() {
    ...
    try {
      const result = await (onSave || defaultAdminSave)(form);
      if (result.ok) {
        ...
        if (!staff) clearDraft();
        setSaved(true);
      } else if (result.alreadyDone && onAlreadyDone) {
        onAlreadyDone();
        return;
      } else {
  ...
  function handleNext() {
    if (step.id === "date" && dateError) return;
    next();
  }
  ...
  if (saved) return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] p-8 text-center">
      ...
      <p className="text-lg font-bold text-gray-800 mb-1">{staff ? "Tak!" : "Done"}</p>
      <p className="text-sm text-gray-800 mb-8">
        {staff
          ? `Kasseopgørelse for ${form.date} er sendt`
          : `${rangeDays > 1 && !singleDayMode ? `${rangeFrom} — ${rangeTo} (${rangeDays} days)` : form.date} saved`}
      </p>
      {staff ? (
        <button onClick={() => (onExit ? onExit() : window.location.reload())} className="w-full max-w-xs py-3.5 bg-brand text-white rounded-xl text-sm font-bold">
          Log ud
        </button>
      ) : (
        <>
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 min-h-0">
        ...
        {step.id === "employee" && (
          <div className="w-full max-w-sm text-center">
            {staff ? (
              <>
                <p className="text-sm text-gray-500 mb-4">Hvem har talt op?</p>
                <input type="text" value={form.employeeName} onChange={(e) => setF("employeeName", e.target.value)} placeholder="Dit navn" autoFocus
      ...
      <div className="flex-shrink-0 border-t bg-white safe-area-bottom">
        <div className="flex items-center px-4 py-3 gap-2 max-w-sm mx-auto">
          {idx > 0 ? (
          ...
          {step.id === "review" ? (
            ...
            <button onClick={() => { setReturnIdx(idx); goStep(STEPS.length - 1); }} className="w-12 h-12 rounded-xl bg-gray-100 text-brand flex items-center justify-center flex-shrink-0" title="Summary">
