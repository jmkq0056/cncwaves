            ...
            <div key={field.name}>
              ...
              {field.type === "textarea" ? (
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-brand resize-none"
                />
              ) : field.type === "select" ? (
                <select
                  value={values[field.name] || ""}
                  onChange={(e) => setValue(field.name, e.target.value)}
                  required={field.required}
                  className="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-brand bg-white"
                >
                ...
                <div>
                  {values[field.name] ? (
                    <div className="flex items-center gap-2">
                      <img src={values[field.name]} alt="" className="w-16 h-16 object-cover rounded border" />
                      <button
                        type="button"
                    ...
                    <label className={`block w-full text-center px-3 py-3 border-2 border-dashed rounded cursor-pointer transition-colors ${
                      ...
                      />
                      {uploadingField === field.name ? "Uploading..." : "Add File"}
                    </label>
                  )}
                </div>
              ) : (
                <input
                  ...
                  placeholder={field.placeholder}
                  required={field.required}
                  className="w-full px-3 py-2 border rounded text-sm focus:outline-none focus:ring-1 focus:ring-brand"
                />
              )}
            </div>
          ))}
